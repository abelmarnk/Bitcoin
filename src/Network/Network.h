...
class Message {
  public:
	...
	virtual void serialize(std::vector<uint8_t>::iterator& start, std::vector<uint8_t>& input,
	                       bool should_adjust_iterator = true) const = 0;
...
class VersionMessage : public Message {
  public:
	...
	VersionMessage(uint32_t version = 70015, uint64_t services = 0,
	               uint64_t timestamp = std::chrono::steady_clock::now().time_since_epoch().count(),
	               uint64_t receiver_services = 0, std::array<uint8_t, 16> receiver_ip = {},
	               uint16_t receiver_port = BITCOIN_TESTNET_PORT, uint64_t sender_services = 0,
	               std::array<uint8_t, 16> sender_ip = {}, uint16_t sender_port = BITCOIN_TESTNET_PORT,
	               uint64_t nonce = std::mt19937_64(std::random_device()())(), std::string user_agent = "",
	               uint32_t latest_block_ = 0, bool relay = false)
	    : version(version), services(services), timestamp(timestamp), receiver_services(receiver_services),
	      receiver_ip(receiver_ip), receiver_port(receiver_port), sender_services(sender_services),
	      sender_ip(sender_ip), sender_port(sender_port), nonce(nonce), user_agent(std::move(user_agent)),
	      latest_block(latest_block), relay(relay) {}

	inline uint32_t get_size() const override {
		return sizeof(version) + sizeof(services) + sizeof(timestamp) + sizeof(receiver_services) +
		       sizeof(receiver_ip) + sizeof(receiver_port) + sizeof(sender_services) + sizeof(sender_ip) +
		       sizeof(sender_port) + sizeof(nonce) + get_varint_byte_size(user_agent.size()) + user_agent.size() +
		       sizeof(latest_block) + (version < 70001 ? 0 : sizeof(relay));
	}

	inline const std::vector<uint8_t>& get_command() const override { return command; }
	...
	void serialize(std::vector<uint8_t>::iterator& start, std::vector<uint8_t>& input,
	               bool should_adjust_iterator = true) const override;

	// Obtains the overlap between the two versions.
	void set_minimum(const VersionMessage& message) { version = std::min(message.version, version); }
  ...
  private:
	...
	uint64_t receiver_final_services; // This is added to store the services the receiver actually provides, in contrast
	                                  // to "receiver_services" which stores services that are expected to be provided.
...
class GetHeadersMessage : public Message {
  public:
	...
	std::array<uint8_t, 32>
	    start_block; // Where to start getting blocks from, this is supposed to be an array,
	...
	std::array<uint8_t, 32>
	    end_block; // Where to stop getting blocks from, if set to zero the max possible(2000) would be returned.

	GetHeadersMessage(int32_t version = 70015, uint64_t hash_count = 0, std::array<uint8_t, 32> start_block = {},
	                  std::array<uint8_t, 32> end_block = {})
	    : version(version), hash_count(hash_count), start_block(std::move(start_block)),
	...
	inline uint32_t get_size() const override {
		return get_varint_byte_size(blocks.size()) + // Size in bytes required to store the block size as a varint
		       std::accumulate(                      // Size of all the blocks
		           blocks.begin(), blocks.end(), 0, [](int sum, const Block& block) { return sum + block.get_size(); });
	}
...
class PingMessage : public Message {
  ...
  public:
	...
	inline void serialize(std::vector<uint8_t>::iterator& start, std::vector<uint8_t>& input,
	                      bool should_adjust_iterator = true) const override {
	...
	inline uint32_t get_size() const override { return sizeof(nonce); }

	void set_nonce(uint64_t nonce_) { nonce = nonce_; }
...
class PongMessage : public Message {
  ...
  public:
	...
	inline uint32_t get_size() const override { return 0; }
...
class MerkleBlockMessage : public Message {
  public:
	...
	inline uint32_t get_size() const override {
		return sizeof(version) + sizeof(prev_block) + sizeof(merkle_root) + sizeof(timestamp) + sizeof(bits) +
		       sizeof(nonce) + sizeof(total_transactions) + hashes.size() * 32 + // Each hash is 32 bytes.
  ...
  public:
	...
	inline uint32_t get_size() const override {
		return sizeof(uint64_t) + // Size of the bit field size
		       ...
		       sizeof(hash_count) + sizeof(tweak) + sizeof(matched_item_flag);
	}
...
class NetworkEnvelope {
  public:
	NetworkEnvelope(std::unique_ptr<Message>&& message, uint32_t magic = TESTNET_NETWORK_MAGIC)
	    : message(std::move(message)), magic(magic) {}
	...
	outcome::result<void, SerializingError> serialize(std::vector<uint8_t>::iterator& start,
	                                                  std::vector<uint8_t>& input,
	                                                  bool should_adjust_iterator = true) const;

	void set_message(std::unique_ptr<Message>&& message) { this->message = std::move(message); }
	...
	void set_magic(uint32_t magic) { this->magic = magic; }

	inline uint32_t get_size() const { return HEADER_SIZE + (message ? message->get_size() : 0); }
...
class NetworkNode {
  public:
	...
	NetworkNode(std::shared_ptr<io_context> ctx, std::string host = "", uint16_t port = 0)
	    : context(ctx), socket(*ctx), host(std::move(host)), port(port), ping_timer(*ctx), ping_deadline_timer(*ctx),
	      acceptor(*ctx, boost::asio::ip::tcp::v4()) {}

	NetworkNode(NetworkNode&& node)
	    : context(node.context), socket(std::move(node.socket)), host(std::move(node.host)), port(node.port),
	      ping_timer(std::move(node.ping_timer)), ping_deadline_timer(std::move(node.ping_deadline_timer)),
	...
	outcome::result<std::unique_ptr<Message>, boost::system::error_code>
	parse_message(const std::vector<uint8_t>& header, const std::vector<uint8_t>& body);
