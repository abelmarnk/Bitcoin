...
inline void VersionMessage::serialize(std::vector<uint8_t>::iterator& start, std::vector<uint8_t>& input,
...
inline void VerAckMessage::serialize(std::vector<uint8_t>::iterator& start, std::vector<uint8_t>& input,
                                     bool should_adjust_iterator) const {
	// The VerAck message has no payload, so this function does nothing.
}
...
inline void GetHeadersMessage::serialize(std::vector<uint8_t>::iterator& start, std::vector<uint8_t>& input,
                                         bool should_adjust_iterator) const {
	// The bytes are serialized in little endian.
	// The iterator is advanced after each write.
...
void HeadersMessage::serialize(std::vector<uint8_t>::iterator& start, std::vector<uint8_t>& input,
                               bool should_adjust_iterator) const {
	// The bytes are serialized in little endian.
	// The iterator is advanced after each write.
...
inline void MerkleBlockMessage::serialize(std::vector<uint8_t>::iterator& start, std::vector<uint8_t>& input,
                                          bool should_adjust_iterator) const {
	// The bytes are serialized in little endian.
	// The iterator is advanced after each write.
...
inline void FilterLoadMessage::serialize(std::vector<uint8_t>::iterator& start, std::vector<uint8_t>& input,
                                         bool should_adjust_iterator) const {
	// The bytes are serialized in little endian.
	// The iterator is advanced after each write.
...
void GetDataMessage::parse(std::vector<uint8_t>::const_iterator&& start) {
	...
	for (uint64_t counter = 0; counter < item_count; ++counter) {
		read_int_from_little_endian_bytes(std::forward<std::vector<uint8_t>::const_iterator>(start),
		                                  items[counter].first);
...
inline void GetDataMessage::serialize(std::vector<uint8_t>::iterator& start, std::vector<uint8_t>& input,
                                      bool should_adjust_iterator) const {
	// The bytes are serialized in little endian.
	// The iterator is advanced after each write.
...
outcome::result<void, SerializingError> NetworkEnvelope::serialize(std::vector<uint8_t>::iterator& start,
...
awaitable<outcome::result<std::unique_ptr<Message>, boost::system::error_code>>
NetworkNode::receive(Command expected_command) {
...
bool NetworkNode::is_command_type(const std::vector<uint8_t>& header, Command expected_command) {
	...
	std::string command(header.begin() + NETWORK_MAGIC_LENGTH, header.begin() + NETWORK_MAGIC_LENGTH + COMMAND_LENGTH);

	return std::equal(header.begin() + NETWORK_MAGIC_LENGTH, header.begin() + NETWORK_MAGIC_LENGTH + COMMAND_LENGTH,
...
NetworkNode::parse_message(const std::vector<uint8_t>& header, const std::vector<uint8_t>& body) {
