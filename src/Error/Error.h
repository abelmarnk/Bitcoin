  ...
  public:
	...
	std::string message(int ev) const override {
		switch (static_cast<NetworkError>(ev)) {
			case NetworkError::CONNECTION_FAILURE:
				return "Connection failure";
			case NetworkError::TIMEOUT:
				return "Operation timed out";
			case NetworkError::INVALID_MESSAGE:
				return "Invalid message format";
			case NetworkError::CHECKSUM_MISMATCH:
				return "Checksum mismatch";
			case NetworkError::UNKNOWN_COMMAND:
				return "Unknown command received";
			case NetworkError::UNEXPECTED_COMMAND:
...
class ArithmeticError : public std::runtime_error {
  public:
	enum class Type { OUT_OF_RANGE, UNDEFINED_OPERATION, DIVISION_BY_ZERO, NULL_POINTER_ACCESS, NUMBER_IS_NULL };
