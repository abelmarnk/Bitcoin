...
class BigFieldElement {
  public:
	...
	bool operator!=(const BigFieldElement& other_element) const { return !(other_element == *this); }
...
class Signature {
  public:
	...
	Signature(const std::vector<uint8_t>& vector_bytes) { std_vec_to_der(vector_bytes); }
...
class BigPoint {
  public:
	BigPoint(const BigFieldElement& a, const BigFieldElement& b, const BigFieldElement& x, const BigFieldElement& y)
	    : a(a), b(b), x(x), y(y), infinity(false) {
		if ((y * y) != ((x * x * x) + (a * x) + b))
	...
	std::vector<uint8_t> compressed_sec_to_std_vec() const { return hex_to_std_vec(compressed_sec_to_hex()); }

	void uncompressed_sec_from_hex(const std::string& sec) {
		return uncompressed_sec_from_std_vec(hex_to_std_vec(sec));
	}

	void compressed_sec_from_hex(const std::string& sec) { compressed_sec_from_std_vec(hex_to_std_vec(sec)); }
...
template <typename T>
concept ValidDigestTag = std::same_as<T, SHA256_tag> || std::same_as<T, HASH256_tag> || std::same_as<T, HASH160_tag>;

template <ValidDigestTag DigestTag> class DigestStream {
  public:
	DigestStream() : context(new_context()), block_size(EVP_MD_block_size(EVP_sha256())) {
		// All hash functions used in this program(Except for murmur hash which isn't used anywhere though) are based
		// off SHA256.
		initialize_context(context.get(), EVP_sha256());
	}

	void update(const std::span<const uint8_t>& data) { update(data.data(), data.size()); }

	template <typename It>
	    requires std::contiguous_iterator<It> && std::integral<std::iter_value_t<It>> &&
	             (sizeof(std::iter_value_t<It>) == 1)
  ...
  private:
	...
	static DigestContextType new_context() {
		...
		if (!context_) {
			throw CryptographyError(CryptographyError::Type::DIGEST_CONTEXT_CREATION_FAILED,
			                        "Digest context creation failed");
		}
	...
	static void initialize_context(EVP_MD_CTX* context, const EVP_MD* digest) {
		if (EVP_DigestInit_ex2(context, digest, nullptr) != 1) {
			throw CryptographyError(CryptographyError::Type::DIGEST_INITIALIZATION_FAILED,
	...
	std::vector<uint8_t> ripemd_160(const std::vector<uint8_t>& input) {
		...
		while (offset < input.size()) {
			size_t chunk = std::min(static_cast<size_t>(block_size), input.size() - offset);
			if (EVP_DigestUpdate(context.get(), input.data() + offset, chunk) != 1) {
				throw CryptographyError(CryptographyError::Type::DIGEST_UPDATE_FAILED,
				                        "RIPEMD160 Digest update failed");
			}
			offset += chunk;
		}
...
const BigFieldElement Secp256k1_x(BigNum("79be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798"),
                                  Secp256k1_Prime);

const BigFieldElement Secp256k1_y(BigNum("483ada7726a3c4655da4fbfc0e1108a8fd17b448a68554199c47d08ffb10d4b8"),
                                  Secp256k1_Prime);
