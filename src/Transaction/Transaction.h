...
class TxIn {
  public:
	...
	TxIn(const TxIn& MyInput)
	    : tx_id(MyInput.tx_id), tx_index(MyInput.tx_index), script_sig(MyInput.script_sig), sequence(MyInput.sequence) {
	}

	TxIn(TxIn&& MyInput) noexcept
	    : tx_id(std::move(MyInput.tx_id)), tx_index(MyInput.tx_index), script_sig(std::move(MyInput.script_sig)),
	...
	void serialize(std::vector<uint8_t>::iterator& start, std::vector<uint8_t>& input,
	...
	const uint32_t get_sequence() const { return sequence; }

	Script& get_script() { return script_sig; }

	void set_script(const Script& NewScript) { script_sig = NewScript; }

	void set_script(Script&& NewScript) { script_sig = std::move(NewScript); }
	...
	std::vector<std::vector<uint8_t>>& get_witness_data() { return witness_data; }

	const std::vector<std::vector<uint8_t>>& get_witness_data() const { return witness_data; }

	void set_witness_data(const std::vector<std::vector<uint8_t>>& witness_data) { this->witness_data = witness_data; }

	const std::array<uint8_t, 32>& get_tx_id() const { return tx_id; }

	std::array<uint8_t, 32>& get_tx_id() { return tx_id; }

	void set_tx_id(const std::array<uint8_t, 32>& tx_id) { this->tx_id = tx_id; }

	uint32_t get_tx_index() const { return tx_index; }

	void set_tx_index(uint32_t tx_index) { this->tx_index = tx_index; }
	...
	uint32_t get_legacy_size() const {
		...
		return sizeof(tx_id) + sizeof(tx_index) + get_varint_byte_size(script_sig_size) + script_sig_size +
		       sizeof(sequence);
	}

	uint32_t get_witness_data_size() const {
		uint32_t witness_data_size = std::accumulate(
		    witness_data.begin(), witness_data.end(), 0LL, [](uint64_t sum, const std::vector<uint8_t>& witness) {
...
class TxOut {
  public:
	TxOut(uint64_t amount, Script script_pubkey) : amount(amount), script_pubkey(script_pubkey) {}
	...
	TxOut(const TxOut& my_output) : amount(my_output.amount), script_pubkey(my_output.script_pubkey) {}

	TxOut(TxOut&& my_output) : amount(my_output.amount), script_pubkey(std::move(my_output.script_pubkey)) {}
	...
	void set_amount(uint64_t amount) { this->amount = amount; }

	void set_script(const Script& script_pubkey) { this->script_pubkey = script_pubkey; }

	void set_script(Script&& script_pubkey) { this->script_pubkey = std::move(script_pubkey); }

	const Script& get_script() const { return script_pubkey; }
...
class Tx {
  public:
	...
	Tx(uint64_t version, const std::vector<TxIn>& input_txs, const std::vector<TxOut>& output_txs, uint32_t locktime,
	   bool testnet, bool segwit)
	    : version(version), input_txs(input_txs), output_txs(output_txs), locktime(locktime), testnet(testnet) {}
	...
	Tx(const Tx& my_tx)
	    : version(my_tx.version), input_txs(my_tx.input_txs), output_txs(my_tx.output_txs), locktime(my_tx.locktime),
	      testnet(my_tx.testnet) {}

	Tx(Tx&& my_tx) noexcept
	    : version(my_tx.version), input_txs(std::move(my_tx.input_txs)), output_txs(std::move(my_tx.output_txs)),
	...
	void serialize(std::vector<uint8_t>& input, std::vector<uint8_t>::iterator& start,
	               bool should_adjust_iterator = true) const;
	...
	void serialize_legacy(std::vector<uint8_t>& input, std::vector<uint8_t>::iterator& start,
	                      bool should_adjust_iterator = true) const;

	void serialize_segwit(std::vector<uint8_t>& input, std::vector<uint8_t>::iterator& start,
	                      bool should_adjust_iterator = true) const;

	std::vector<TxIn>& get_in_txs() { return input_txs; }

	void set_in_txs(const std::vector<TxIn>& input_txs) { this->input_txs = input_txs; }

	void set_in_txs(std::vector<TxIn>&& input_txs) { this->input_txs = std::move(input_txs); }

	std::vector<TxOut>& get_out_txs() { return output_txs; }

	void set_out_txs(const std::vector<TxOut>& NewOutputTx) { output_txs = NewOutputTx; }

	void set_out_txs(std::vector<TxOut>&& NewOutputTxs) { output_txs = std::move(NewOutputTxs); }

	uint64_t get_version() const { return version; }

	void set_version(uint64_t NewVersion) { version = NewVersion; }

	uint32_t get_locktime() const { return locktime; }

	void set_locktime(uint32_t NewLocktime) { locktime = NewLocktime; }

	bool is_testnet() const { return testnet; }

	void set_testnet(bool NewTestnet) { testnet = NewTestnet; }
	...
	int64_t get_total_fee() const { return get_input_sum() - get_output_sum(); }
	...
	std::vector<uint8_t> get_bip143_transaction_hash(uint64_t input_index, std::vector<uint8_t>& script_code,
	                                                 int64_t script_size = -1);
	...
	bool is_segwit() const {
		auto witness_data_size =
		    std::accumulate(input_txs.begin(), input_txs.end(), 0ULL, // Add witness data.
		                    [](uint64_t sum, const TxIn& input) { return sum + input.get_witness_data_size(); });
	...
	uint64_t get_legacy_size() const {
		return sizeof(version) + get_varint_byte_size(input_txs.size()) + // Size of the varint for the number of inputs
	...
	uint64_t get_segwit_size() const {
		return get_legacy_size() + 1 +                                   // Segwit indicator.
		       1 +                                                       // Segwit marker.
		       std::accumulate(input_txs.begin(), input_txs.end(), 0ULL, // Add witness data.
		                       [](uint64_t sum, const TxIn& input) {
			                       auto witness_data_size = input.get_witness_data_size();
			                       return sum + (witness_data_size == 0 ? 1 : witness_data_size);
		                       }); // If the input has no witness set it to zero.
	}
...
class TxInfoFetcher {
  private:
	static std::unordered_map<std::string, std::pair<nlohmann::json, uint32_t>> cache;
	...
	static const nlohmann::json& fetch_tx(std::string&& tx_id, bool IsTestnet = true);
