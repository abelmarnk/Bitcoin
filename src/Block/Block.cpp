...
void Block::serialize(std::vector<uint8_t>::iterator& start, std::vector<uint8_t>& input,
...
uint32_t Block::target_to_bits(const std::vector<uint8_t>& target) {
	...
	if (coefficient & 0x00800000) {
		...
		// If the target was derived to have a block created against it, the miner would already have it at hand and
		// work with it.
		coefficient >>= 8;
		exponent += 1;
	}
...
std::vector<uint8_t> Block::calculate_new_target(const Block& first_block, const Block& last_block) {
	...
	// We reduce possible rounding errors by converting to target, using Openssl and performing the multiplication
	// first.
	return ((BigNum(last_block.bits_to_target(last_block.bits)) * BigNum(time_differential)) / TWO_WEEKS)
	    .to_std_vec(); // Calculate the new bits.
}