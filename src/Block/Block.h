...
class Block {
  public:
	...
	Block(uint32_t version, const std::array<uint8_t, 32>& prev_block, const std::array<uint8_t, 32>& merkle_root,
	      uint32_t timestamp, uint32_t bits, uint32_t nonce)
	    : version(version), prev_block(prev_block), merkle_root(merkle_root), timestamp(timestamp), bits(bits),
	      nonce(nonce) {}
	...
	bool bip_9() const { return (version & 0xE0000000) == 0x20000000; }

	// Check support for bip 91
	bool bip_91() const { return (version & 0x10) != 0; }

	// Check support for bip 141
	bool bip_141() const { return (version & 0x2) != 0; }
