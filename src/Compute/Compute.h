...
class BigNum {
  public:
	// Context deleter.
	void static free_big_number_context(BN_CTX* NumberContext) { BN_CTX_free(NumberContext); }

	// Number representation deleter.
	void static free_big_number(BIGNUM* Number) { BN_free(Number); }
	...
	template <std::integral T> BigNum(T number) : number(nullptr, &free_big_number) {
		...
		std::reverse(vector_bytes.begin(),
		             vector_bytes.end()); // The "write_int_as_little_endian_bytes" function extracts the bytes in
		                                  // little-endian but Openssl uses big-endian.

		this->number = std::move(NumberType(
		    BN_bin2bn(vector_bytes.data(), static_cast<int32_t>(vector_bytes.size()), nullptr), &free_big_number));
	...
	bool operator<(const BigNum& OtherElement) const { return (BN_cmp(number.get(), OtherElement.number.get()) == -1); }

	bool operator>(const BigNum& OtherElement) const { return (BN_cmp(number.get(), OtherElement.number.get()) == 1); }

	bool operator==(const BigNum& OtherElement) const { return (BN_cmp(number.get(), OtherElement.number.get()) == 0); }

	bool operator<=(const BigNum& OtherElement) const { return (*this < OtherElement || *this == OtherElement); }

	bool operator>=(const BigNum& OtherElement) const { return (*this > OtherElement || *this == OtherElement); }

	bool operator!=(const BigNum& OtherElement) const { return !(OtherElement == *this); }

	BigNum operator+(const BigNum& OtherElement) const { return BigNum(add(number.get(), OtherElement.number.get())); }

	BigNum operator-(const BigNum& OtherElement) const { return BigNum(sub(number.get(), OtherElement.number.get())); }

	BigNum operator-() const { return BigNum(0) - *this; }

	BigNum operator*(BigNum Element) const { return BigNum(mul(number.get(), Element.number.get())); }

	BigNum operator/(BigNum Element) const { return BigNum(div(number.get(), Element.number.get())); }

	BigNum operator^(BigNum Element) const { return BigNum(exp(number.get(), Element.number.get())); }
	...
	BigNum operator%(BigNum Element) const { return BigNum(mod(number.get(), Element.number.get())); }

	void negate() { BN_set_negative(number.get(), BN_is_negative(number.get()) == 0 ? 1 : 0); }

	void set_absolute() { BN_set_negative(number.get(), 0); }
	...
	bool is_unsigned_small() const {
		return BN_num_bytes(number.get()) <=
		           8 && // Check if it can be stored in C++'s primitive types.
		                // Checks if it is within the bounds of the largest primitive type that C++ offers
	...
	// but Openssl probably truncates the leading bytes.
	int64_t get_unsigned_small() const { return BN_get_word(number.get()); }

	uint64_t byte_count() const { return BN_num_bytes(number.get()); }
