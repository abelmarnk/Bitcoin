...
bool OP_add(std::stack<ScriptInput>& script_inputs) {
	...
	script_inputs.push(ScriptInput((BigNum(Message_1.get_value()) + BigNum(Message_2.get_value())).to_std_vec()));
...
void Script::initialize_opcode_functions() {
	opcode_functions.emplace(ScriptInput::OpCode::OP_DUP,
	                         std::make_unique<OpCodeFunction<bool, std::stack<ScriptInput>&>>(OP_dup));
	opcode_functions.emplace(ScriptInput::OpCode::OP_ADD,
	                         std::make_unique<OpCodeFunction<bool, std::stack<ScriptInput>&>>(OP_add));
	opcode_functions.emplace(ScriptInput::OpCode::OP_SUB,
	                         std::make_unique<OpCodeFunction<bool, std::stack<ScriptInput>&>>(OP_sub));
	opcode_functions.emplace(ScriptInput::OpCode::OP_EQUALVERIFY,
	                         std::make_unique<OpCodeFunction<bool, std::stack<ScriptInput>&>>(OP_equal_verify));
	opcode_functions.emplace(ScriptInput::OpCode::OP_EQUAL,
	                         std::make_unique<OpCodeFunction<bool, std::stack<ScriptInput>&>>(OP_equal));
	opcode_functions.emplace(ScriptInput::OpCode::OP_HASH160,
	                         std::make_unique<OpCodeFunction<bool, std::stack<ScriptInput>&>>(OP_hash160));
	opcode_functions.emplace(ScriptInput::OpCode::OP_HASH256,
	                         std::make_unique<OpCodeFunction<bool, std::stack<ScriptInput>&>>(OP_hash256));
	opcode_functions.emplace(
	    ScriptInput::OpCode::OP_CHECKSIG,
	    std::make_unique<OpCodeFunction<bool, std::stack<ScriptInput>&, const std::vector<uint8_t>&>>(OP_checksig));
	opcode_functions.emplace(
	    ScriptInput::OpCode::OP_CHECKMULTISIG,
	    std::make_unique<OpCodeFunction<bool, std::stack<ScriptInput>&, const std::vector<uint8_t>&>>(
	        Op_checkmultisig));
}

void Script::serialize(std::vector<uint8_t>& input, std::vector<uint8_t>::iterator& start,
			...
			*start = static_cast<uint8_t>(input_iterator->get_opcode());
			...
			if (input_iterator->get_value().size() <= 75) {
				...
				*start = static_cast<uint8_t>(input_iterator->get_value().size());
				start++;
			} else if (input_iterator->get_value().size() > 75 && input_iterator->get_value().size() < 256) {
				// Insert 76 and the byte count of the new ScriptInput and advance the iterator and increase the size.
				*start = 76;
				start++;
				*start = static_cast<uint8_t>(input_iterator->get_value().size());
				start++;
			} else if (input_iterator->get_value().size() >= 256 && input_iterator->get_value().size() < 512) {
...
bool Script::evaluate(const std::vector<uint8_t>& Hash, const std::vector<std::vector<uint8_t>>& witness_data) {
	...
	while (temp_inputs != inputs.end()) {
		...
		if (is_p2sh(temp_inputs, inputs.end())) { // https://learnmeabitcoin.com/technical/script/p2sh/
			...
			std::copy(std::make_move_iterator(temp_remaining_inputs.begin()),
			          std::make_move_iterator(temp_remaining_inputs.end()), std::back_inserter(inputs));
			...
			// The last witness item is the witness script (redeem script).
			const auto& witness_script = witness_data.back();

			// Verify: SHA256(witness_script) == witness_hash from scriptPubKey.
			auto computed_hash = get_sha_256(witness_script);
			if (computed_hash != witness_hash) {
				inputs.resize(current_size);
				return false;
			}

			// Skip over p2wsh inputs (OP_0 + 32-byte hash).
			temp_inputs += 2;
			...
			// Push all witness items except the last (which is the witness script)
			// onto the operand stack as data for the script to consume.
			for (size_t i = 0; i < witness_data.size() - 1; ++i) {
				operand_stack.push(ScriptInput(witness_data[i]));
			}

			// Parse and append the witness script for execution.
			append_parse_from_non_prefixed_size(witness_script.cbegin(), witness_script.size());
			...
			if (temp_input.get_opcode() != ScriptInput::OpCode::OP_0) {
				...
				if (static_cast<uint8_t>(temp_input.get_opcode()) >= static_cast<uint8_t>(ScriptInput::OpCode::OP_1) &&
					...
					if (static_cast<uint8_t>(temp_input.get_opcode()) >= 99 &&
					    static_cast<uint8_t>(temp_input.get_opcode()) <= 100) {
						// No instruction in this range is supported by this program.
						throw ScriptError(ScriptError::Type::OPCODE_NOT_SUPPORTED);
					} else if (static_cast<uint8_t>(temp_input.get_opcode()) >= 107 &&
					           static_cast<uint8_t>(temp_input.get_opcode()) <= 108) {
						// No instruction in this range is supported by this program.
						throw ScriptError(ScriptError::Type::OPCODE_NOT_SUPPORTED);
					} else if (static_cast<uint8_t>(temp_input.get_opcode()) >= 172 &&
					           static_cast<uint8_t>(temp_input.get_opcode()) <= 175) {
						auto result =
						    dynamic_cast<OpCodeFunction<bool, std::stack<ScriptInput>&, const std::vector<uint8_t>&>*>(
						        temp_function);
						if (result) {
	...
	inputs.resize(current_size);
	return true; // This does not conform to the implementation in the actual Bitcoin code, but it is used here because,
	             // following the implementation, without changing a lot of other thing that are also different from the
	             // standard would cause errors based on the way it is implemented here, it could also falsely return
	             // true if data was placed in the right places instead of opcodes, e.g if only data was placed in the
	             // stack, it would return true.
}
...
Script create_p2ms_out(uint8_t signature_count, uint8_t pubkey_count,
	...
	if (signature_count < 1 || signature_count > pubkey_count || pubkey_count < 1 || pubkey_count > 3 ||
	    public_keys.size() != pubkey_count) {
		throw std::invalid_argument("Invalid signature or public key count for multisig script.");
	}

	inputs.push_back(ScriptInput(
	    static_cast<ScriptInput::OpCode>(signature_count + static_cast<uint8_t>(ScriptInput::OpCode::OP_1))));
	for (const auto& pubkey : public_keys) {
		inputs.push_back(ScriptInput(pubkey));
	}
	inputs.push_back(
	    ScriptInput(static_cast<ScriptInput::OpCode>(pubkey_count + static_cast<uint8_t>(ScriptInput::OpCode::OP_1))));
...
bool is_p2sh(std::deque<ScriptInput>::const_iterator iterator, std::deque<ScriptInput>::const_iterator end) {
	...
	return (it[0] == ScriptInput::OpCode::OP_HASH160) && (it[1].get_value().size() == 20) &&
...
bool is_p2wpkh(std::deque<ScriptInput>::const_iterator iterator, std::deque<ScriptInput>::const_iterator end) {
	...
	return (it[0] == ScriptInput::OpCode::OP_0) && (it[1].get_value().size() == 20);
}
...
bool is_p2wsh(std::deque<ScriptInput>::const_iterator iterator, std::deque<ScriptInput>::const_iterator end) {
	...
	return (it[0] == ScriptInput::OpCode::OP_0) && (it[1].get_value().size() == 32);
}
