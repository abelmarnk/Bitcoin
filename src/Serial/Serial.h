...
template <std::size_t Size> std::array<uint8_t, Size> hex_to_std_array(const std::string& hex) {
	if (hex.size() % 2 != 0) {
	...
	auto hex_to_byte = [](char c) -> uint8_t {
		...
		if (c >= 'A' && c <= 'F')
			return c - 'A' + 10;
		throw ParsingError(ParsingError::Type::INVALID_DATA, "Invalid hex character.");
	};
...
template <std::size_t Size> inline std::string std_array_to_hex(const std::array<uint8_t, Size>& array_bytes) {
...
template <std::integral T> T read_int_from_little_endian_bytes(std::vector<uint8_t>::const_iterator& iterator) {
...
void write_int_as_big_endian_bytes(T value, std::vector<uint8_t>::iterator& iterator, std::vector<uint8_t>& vector) {
	...
	for (std::size_t counter = 0; counter < sizeof(T); ++counter) {
		*iterator = static_cast<uint8_t>(value >> (8 * ((sizeof(T) - 1) - counter))); // Extract the bytes and put it in
		                                                                              // the appropriate position.
		iterator++;
	}
}

template <std::integral T> std::vector<uint8_t> base_int_to_big_endian_bytes(T value, bool pad_to_size) {
...
template <std::integral T> std::vector<uint8_t> base_int_to_little_endian_bytes(T value, bool pad_to_size) {
...
template <std::integral T> std::vector<uint8_t> int_to_big_endian_bytes_pad(T value) {
	return base_int_to_big_endian_bytes<T>(value, true);
}

template <std::integral T> std::vector<uint8_t> int_to_big_endian_bytes_no_pad(T value) {
	return base_int_to_big_endian_bytes<T>(value, false);
}

template <std::integral T> std::vector<uint8_t> int_to_little_endian_bytes_pad(T value) {
	return base_int_to_little_endian_bytes<T>(value, true);
}

template <std::integral T> std::vector<uint8_t> int_to_little_endian_bytes_no_pad(T value) {
	return base_int_to_little_endian_bytes<T>(value, false);
}

template <std::integral T> T little_endian_bytes_to_int(const std::vector<uint8_t>& bytes) {
...
template <std::integral T> T big_endian_bytes_to_int(const std::vector<uint8_t>& bytes) {
