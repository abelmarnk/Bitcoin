...
TEST_CASE("Script test", "[script][serialization][op_checksig]") {
	...
	std::string private_key_hex = GENERATE("99f2882e", "12345678", "ea", "433423234aea");

	std::vector<uint8_t> message =
	    GENERATE(std::vector<uint8_t>{1, 2, 3, 4, 5, 6, 7, 8}, std::vector<uint8_t>{9, 8, 7, 6, 5, 4, 3, 2},
	             std::vector<uint8_t>{0xff, 0xee, 0xdd}, std::vector<uint8_t>{0, 1, 0, 1, 0, 1});
...
TEST_CASE("Script test", "[script][serialization][op_add][op_sub]") {
	...
	ScriptInput input_1 = GENERATE(int_to_big_endian_bytes_no_pad(10), int_to_big_endian_bytes_no_pad(20),
	                               int_to_big_endian_bytes_no_pad(30), int_to_big_endian_bytes_no_pad(40));

	ScriptInput input_2 = GENERATE(int_to_big_endian_bytes_no_pad(40), int_to_big_endian_bytes_no_pad(30),
	                               int_to_big_endian_bytes_no_pad(20), int_to_big_endian_bytes_no_pad(10));

	ScriptInput input_3 = GENERATE(ScriptInput::OpCode::OP_SUB, ScriptInput::OpCode::OP_ADD);
