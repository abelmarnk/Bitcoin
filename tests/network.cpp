...
awaitable<std::optional<NetworkNode>> test_handshake(std::shared_ptr<boost::asio::io_context> context,
                                                     const std::vector<std::pair<std::string, uint16_t>>& seeds) {
	...
	for (const auto& [host, port] : seeds) {
		try {
			...
			co_spawn(
			    ...
			    redirect_error(
			        boost::asio::bind_cancellation_slot(handshake_cancel_signal.slot(), boost::asio::detached),
			        handshake_error));

			boost::asio::steady_timer timer(co_await boost::asio::this_coro::executor);
			timer.expires_after(std::chrono::seconds(15));
			co_await timer.async_wait(redirect_error(
			    boost::asio::bind_cancellation_slot(timer_cancel_signal.slot(), use_awaitable), timer_error));
			...
			if (handshake_result.value()) {
				std::cerr << "Handshake failed with " << host << ":" << port << " ("
				          << handshake_result.value().message() << ")" << std::endl;
				continue;
			}
			...
			std::cerr << "Exception during handshake with " << host << ":" << port << std::endl;
		}
	}
...
TEST_CASE("Network", "[handshake][network]") {
	...
	std::vector<std::pair<std::string, uint16_t>> seeds = {
	    ...
	    {"testnet-seed.breadwallet.com", 18333},
	};
